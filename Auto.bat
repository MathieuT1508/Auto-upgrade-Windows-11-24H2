@Echo off & title Auto Upgrade - MCT
...
if %errorLevel% == 0 (
    ...
    color 4f
    Echo Échec : Permissions administratives insuffisantes.
    Echo VOUS DEVEZ ÊTRE UN ADMINISTRATEUR
    Echo.
    pause & exit
)
...
set OPTIONS=%OPTIONS% /ShowOOBE None /Telemetry Disable /CompactOS Enable /DynamicUpdate Disable /SkipSummary /Eula Accept
...
::# Registry Portable mode
set "K=HKLM\SYSTEM\CurrentControlSet\Control"
set "V=PortableOperatingSystem"
reg query "%K%" /v "%V%" >nul 2>&1
if %errorlevel%==0 reg delete "%K%" /v "%V%" /f
...
for %%e in (Ultimate ProfessionalStudent ProfessionalCountrySpecific ProfessionalSingleLanguage) do if /i %%e equ %eID% (set "eID=Professional") else if /i %%eN equ %eID% set "eID=ProfessionalN"
for %%e in (EnterpriseG EnterpriseS IoTEnterpriseS IoTEnterprise Embedded) do if /i %%e equ %eID% (set "eID=Enterprise") else if /i %%eN equ %eID% set "eID=EnterpriseN"
for %%e in (Enterprise EnterpriseS) do if /i %%eEval equ %eID% (set "eID=Enterprise") else if /i %%eNEval equ %eID% set "eID=EnterpriseN"
...
reg add "HKLM\SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate" /f /v DisableWUfBSafeguards /d 1 /t REG_DWORD >nul 2>&1
...
if "%SKIP_11_SETUP_CHECKS%" equ "1" cd.>appraiserres.dll ::# Writable media only
...
:Rename ::# EditionID
set "NT=HKLM\SOFTWARE\Microsoft\Windows NT\CurrentVersion"
for %%v in (CompositionEditionID EditionID ProductName) do reg add "%NT%" /v %%v_undo /d "!%%v!" /f >nul 2>&1
for %%A in (32 64) do (
...
reg delete "%App%\CompatMarkers" /f >nul 2>&1
reg delete "%App%\Shared" /f >nul 2>&1
reg delete "%App%\TargetVersionUpgradeExperienceIndicators" /f >nul 2>&1
reg add "%App%\HwReqChk" /v HwReqChkVars /t REG_MULTI_SZ /s , /d "%Config%" /f >nul 2>&1
for %%s in (sCPU sRAM sSecureBoot sStorage sTPM sDisk) do reg add HKLM\SYSTEM\Setup\LabConfig /f /v Bypas%%sCheck /d 1 /t REG_DWORD >nul 2>&1
reg add HKLM\SYSTEM\Setup\MoSetup /f /v AllowUpgradesWithUnsupportedTPMorCPU /d 1 /t REG_DWORD >nul 2>&1 ::# TPM 1.2+ only
exit /b
